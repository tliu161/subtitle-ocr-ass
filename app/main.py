...
class ROISelectorApp:
    def __init__(self, root):
        ...
        tk.Button(top, text="Select Video", command=self.select_video).pack(side="left")
        tk.Button(top, text="Load Preview", command=self.load_preview).pack(side="left", padx=8)
        tk.Button(top, text="Run OCR → ASS", command=self.run_ocr).pack(side="left", padx=8)
        ...
        self.status = tk.Label(bottom, text=self._status_text())
